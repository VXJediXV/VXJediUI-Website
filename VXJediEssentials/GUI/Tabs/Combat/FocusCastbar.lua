...
local function GetFocusCastbarModule()
    if VXJediEssentials then
        return VXJediEssentials:GetModule("FocusCastbar", true)
    end
    return nil
end

GUIFrame.CastbarBuilders = GUIFrame.CastbarBuilders or {}
GUIFrame.CastbarBuilders.Focus = function(scrollChild, yOffset)
    local db = AE.db and AE.db.profile.Miscellaneous.FocusCastbar
    if not db then
    ...
    local function ApplyFocusCastbarState(enabled)
        ...
        if enabled then
            VXJediEssentials:EnableModule("FocusCastbar")
        else
            VXJediEssentials:DisableModule("FocusCastbar")
        end
    end
    ...
    ----------------------------------------------------------------
    local card1 = GUIFrame:CreateCard(scrollChild, L["Focus Castbar"], yOffset)

    local row1 = GUIFrame:CreateRow(card1.content, 36)
    local enableCheck = GUIFrame:CreateCheckbox(row1, L["Enable Focus Castbar"], db.Enabled ~= false,
        function(checked)
            db.Enabled = checked
            ApplyFocusCastbarState(checked)
            UpdateAllWidgetStates()
        end,
        true, L["Focus Castbar"], L["On"], L["Off"]
    )
