...
function GUIFrame:CreateSectionHeader()
    local ARROW_SIZE = 16
    local arrowTex = "Interface\\AddOns\\VXJediEssentials\\Media\\GUITextures\\collapse.tga"
    ...
    arrow:Hide() -- Categories always expanded
    header.arrow = arrow
    ...
    -- Hover effects (disabled - sections always expanded)

    -- Click handler (disabled - sections always expanded)
...
function GUIFrame:ConfigureSectionHeader(header, config, yOffset, isExpanded)
    ...
    if config.elvUIDisabled and AE:ShouldNotLoadModule() then
...
function GUIFrame:CreateSidebar(parent)
    ...
    sidebar:SetWidth(Theme.sidebarWidth)
    ...
    for _, sectionConfig in ipairs(config) do
        if sectionConfig.type == "header" then
            ...
            if isExpanded and sectionConfig.items then
                local sectionDisabled = sectionConfig.elvUIDisabled and AE:ShouldNotLoadModule()
