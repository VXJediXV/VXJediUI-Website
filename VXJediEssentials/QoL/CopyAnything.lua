...
local CopyAnything = VXJediEssentials:NewModule("CopyAnything", "AceEvent-3.0")
...
function CopyAnything:UpdateDB()
    self.db = AE.db.profile.Miscellaneous.CopyAnything
end
...
-- Show the copy dialog with the given spell/item ID and name.
-- Uses AE:CreatePrompt in display-only mode (editbox auto-highlighted,
-- Ctrl+C copies and closes).
...
function CopyAnything:TryCopy(key)
    ...
    -- OnKeyDown fires on every single keystroke, so we want this path to be cheap
...
function CopyAnything:OnEnable()
    ...
    if not self.frame then
        self.frame = CreateFrame("Frame", "AE_CopyFrame")
